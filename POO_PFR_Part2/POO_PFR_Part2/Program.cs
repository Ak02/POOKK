        ...
        public static void Chargement_Personnel_Attraction(string chemin)
        {
            ...
            while (ligne != null)
            {
                try
                {
                    ...
                    if (tmp[0] == "Sorcier")
                    {
                       // personnelS.Add(new Sorcier(int.Parse(tmp[1]), tmp[2], tmp[3], (Personnel.TypeSexe)Enum.Parse(typeof(Personnel.TypeSexe),tmp[4]), tmp[5], tmp[6], (Sorcier.Grade)Enum.(typeof(Sorcier.Grade),tmp[7]) );
